...
mod tests {
    ...
    fn test_message_parser_returns_hex_decode_error() {
        ...
        assert!(!result.ok);
        assert_eq!(result.parse_error.as_deref(), Some("Hex payload 长度必须为偶数"));
    }
    ...
    fn execute_message_parser_supports_extended_helpers() {
        let input = MessageParserRuntimeInput {
            ...
            dup: false,
            bytes: vec![
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x3f, 0x80, 0x00, 0x00, 0xe4,
                0xbd, 0xa0, 0xe5, 0xa5, 0xbd, 0x68,
            ],
        };
